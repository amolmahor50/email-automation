...
const createTemplate = async (req, res) => {
  try {
    const { title, body, category, visibility, defaultAttachments, tags } = req.body;
...
const updateTemplate = async (req, res) => {
  try {
    ...
    const template = await Template.findOne({
      ...
      $or: [
        { ownerId: req.user.id },
        { visibility: 'global', ownerId: null }
      ]
    });
    ...
    if (template.visibility === 'global' && req.user.role !== 'admin') {
      return res.status(403).json({ message: 'Only admins can update global templates' });
    }

    // Only admins can change visibility to global
    if (visibility === 'global' && req.user.role !== 'admin') {
      return res.status(403).json({ message: 'Only admins can create global templates' });
    }
    ...
    template.defaultAttachments = defaultAttachments || template.defaultAttachments;
...
const deleteTemplate = async (req, res) => {
  try {
    ...
    if (template.visibility === 'global' && req.user.role !== 'admin') {
      return res.status(403).json({ message: 'Only admins can delete global templates' });
    }
